...
//Start session
	...
	$g = mysqli_query($dbc, "SELECT * FROM generos WHERE genero_id='".$_GET['gen_id']."'");
	$g_row = mysqli_fetch_array($g);
	
	$c = mysqli_query($dbc, "SELECT * FROM categorias WHERE categoria_id='".$g_row['cat_id']."'");
	...
	<body>
		...
		<main class="mt-4 p-5 text-dark">
			<div class="container-fluid">
				...
				<form method='post' action='genero_edit.php'>
					<input type='hidden' name='gen_id' value='<?php echo $_GET["gen_id"];?>'>
					<label for='sel_cat' class='form-label'>Categoria:</label>
					<select class='form-select' id='sel_cat' name='cat_id'>
						<option value='<?php echo $c_row["categoria_id"] ?>'><?php echo $c_row['categoria'] ?></option>
						<?php $nc = mysqli_query($dbc, "SELECT * FROM categorias WHERE categoria_id <> '".$g_row['cat_id']."'ORDER BY categoria ASC");
						while($nc_row = mysqli_fetch_array($nc)){?>
							<option value='<?php echo $nc_row["categoria_id"] ?>'><?php echo $nc_row['categoria'] ?></option>
						<?php } ?>
					</select><br>
					<p>Genero: <input type='text' name='genero' size='50' maxlength='50' value='<?php echo $g_row["genero"] ?>'/></p>
